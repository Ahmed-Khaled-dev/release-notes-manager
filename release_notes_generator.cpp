...
// "Automated release notes generator" project, the script generates release notes from conventional git commits
...
// Add function header
void generateReleaseNotes(Commits commit, OutputTypes outputType){
    ofstream outputFile;
    ...
    for (int i = 0; i < COMMIT_TYPES_COUNT; i++)
    {
        ...
        if(commit == Commits::Normal){
            // Need to adjust this command to remove commit ids and other useless stuff
            commandToRetrieveCommitsMessages = "git log --max-count " + to_string(MAX_DISPLAYED_COMMITS_PER_TYPE) + 
            " --no-merges --oneline --format=\"%s\" --grep=\"^" + commitTypes[i][(int) CommitTypeInfo::ConventionalName] + ": \"";

            //cout << commandToRetrieveCommitsMessages.c_str() << endl;
            FILE* pipe = popen(commandToRetrieveCommitsMessages.c_str(), "r");
            if(!pipe){
                cout << "Error: Unable to open pipe to read git log commmand output" << endl;
            }
            ...
            while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
                ...
                if(currentCommitMessage.substr(0, currentCommitMessage.find(":")) == commitTypes[i][(int) CommitTypeInfo::ConventionalName])
                ...
                outputFile << currentCommitMessage;
            }
        }
    }
}
...
int main(int argc, char* argv[]){
    if(argc > 1){
        ...
        generateReleaseNotes(Commits::Normal, OutputTypes::Markdown);
            ...
            generateReleaseNotes(Commits::Merge, OutputTypes::Markdown);
        }
        else{
            // Can enhance this part by using options -s, -f, etc. 
            // And also check git and other popular tools on how to send better help messages
